...
namespace ITManagementSystemWebAPI.Controllers
{
    ...
    public class TakeLeaveController : ODataController
    {
        ...
        public TakeLeaveController(IMapper mapper)
        {
            this.takeLeaveRepository = new TakeLeaveRepository();
        ...
        {
            var tempContract = contractRepository.GetActiveContractByEmployeeIdEqual(postTakeLeave.EmployeeId);
        ...
        public IActionResult Patch([FromRoute] int key, [FromBody] Delta<TakeLeave> delta)
        {
            ...
            var tempContract = contractRepository.GetActiveContractByEmployeeIdEqual(takeLeave.EmployeeId);
            if (tempContract == null)
            {
                return BadRequest("Require at least 1 active contract to edit leave!");
            }

            if (delta.TryGetPropertyValue("StartDate", out _) || delta.TryGetPropertyValue("EndDate", out _))
            {
